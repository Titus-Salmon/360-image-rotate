jQuery.fn.rotate360 = function (settings) {
	...
	var options = {
		yourDirectory: settings.yourDirectory || "A_STATION_1tone",
	...
	function change_image(number) {
		...
		$("#bg_smooth").attr("src", options.yourDirectory.toLowerCase() + '/a_360_' + prev_image + '.jpg');
		$(self).css("background-image", "url(" + options.yourDirectory.toLowerCase() + "/a_360_" + number + ".jpg)");
	}
	...
	/**'.live()' is deprecated -- use '.on()' -- see below */

	$('body').on('slide', '#slider', function (event, ui) {
		var value = ui.value;
		change_image(value < 10 ? "0" + value : value);
	});
	/** $("#slider").live("slide", function (event, ui) {
		var value = ui.value;
		change_image(value < 10 ? "0" + value : value);
	});*/

	$('body').on('click', '#main_image_container div.play', function (e) {
		e.preventDefault();
		autoplay(false);
	});
	...
	/**'.live() is deprecated. mjst use .on() -- see above */

	function preload_image() {
		var src = options.yourDirectory + "/" + options.images[image_preloader_controller].name;
		current_image_being_preloaded = new Image();
		current_image_being_preloaded.src = src;

		//$(selector).load(function(){}) is deprecated. must change to $(selector).on('load', function(){ ...});
		...
		$(current_image_being_preloaded).on('load', function () {
			$("#progress_bar").progressbar(Math.floor((image_preloader_controller / options.images.length) * 100));
			if ((image_preloader_controller + 1) < options.images.length) {
				image_preloader_controller++;
				preload_image();
